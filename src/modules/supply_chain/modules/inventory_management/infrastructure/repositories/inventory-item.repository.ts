    ...
    return await this.db.transaction(async () => {
      ...
      while (retry > 0) {
        try {
          ...
          const inventoryRecord = await this.db.client.inventoryItem.findFirst({
            orderBy: {
              id: "desc",
            },
            select: {
          ...
          if (!inventoryRecord) {
            ...
            const codeNumber = Number(inventoryRecord.code.split("-")[1]);
            if (codeNumber <= 9) {
          ...
          inventoryItem = await this.db.client.inventoryItem.create(data);
        } catch (error) {
